...
#             energy_i = - \
#                 sum(np.cos(self.spin_config[idx] - self.spin_config[n]) for n in self.nbr[idx])
#             dtheta = np.random.uniform(-np.pi, np.pi)
#             spin_temp = self.spin_config[idx] + dtheta
#             energy_f = - \
#                 sum(np.cos(spin_temp - self.spin_config[n]) for n in self.nbr[idx])
...
#     # sample/'log' or 'lin',mean linear sampled T or log sampled( centered at
#     # critical point)
